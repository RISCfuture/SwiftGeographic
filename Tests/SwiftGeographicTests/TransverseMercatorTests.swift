...
@Suite
struct `TransverseMercator Projection Tests` {
  ...
  @Test
  func `Forward projection at equator and central meridian yields (0, 0)`() {
  ...
  @Test
  func `Scale at central meridian equals k0 (0.9996)`() {
    let result = tm.forward(centralMeridian: 0, latitude: 0, longitude: 0)
    #expect(result.scale.isApproximatelyEqual(to: 0.9996, absoluteTolerance: 1e-10))
  }

  @Test
  func `Convergence at central meridian is zero`() {
  ...
  @Test
  func `Forward projection of New York City (40.7128, -74.006) with CM -75`() {
  ...
  @Test
  func `Forward projection at various latitudes on central meridian`() {
  ...
  @Test
  func `Forward projection yields increasing northing with latitude`() {
  ...
  @Test
  func `Forward-reverse round trip within 1e-9 degrees`() {
  ...
  @Test
  func `Reverse projection at origin yields (0, CM)`() {
  ...
  @Test
  func `Reverse projection scale matches forward scale`() {
  ...
  @Test
  func `Reverse projection convergence matches forward convergence`() {
    let fwd = tm.forward(centralMeridian: -75, latitude: 40.7128, longitude: -74.006)
    let rev = tm.reverse(centralMeridian: -75, easting: fwd.x, northing: fwd.y)
  ...
  @Test
  func `Forward projection at negative latitudes`() {
    let result = tm.forward(centralMeridian: 0, latitude: -45, longitude: 0)
...
@Suite
struct `TMcoords.dat Validation (287K points)` {
  ...
  @Test
  func `Forward projection within convergence zone matches reference within 15 nm`() throws {
  ...
  @Test
  func `Reverse projection within convergence zone matches reference within 1e-9 deg`() throws {
    let all = try Self.loadTestVectors()
    let vectors = Self.convergenceZoneVectors(from: all)
  ...
  @Test
  func `Scale within convergence zone matches reference within 1e-12 relative error`() throws {
    let all = try Self.loadTestVectors()
    let vectors = Self.convergenceZoneVectors(from: all)
  ...
  @Test
  func `Convergence within convergence zone matches reference within 1e-9 deg`() throws {
    let all = try Self.loadTestVectors()
    let vectors = Self.convergenceZoneVectors(from: all)
